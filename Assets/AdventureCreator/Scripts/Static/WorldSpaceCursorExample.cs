...
namespace AC
{
	...
	[HelpURL("http://www.adventurecreator.org/scripting-guide/class_a_c_1_1_world_space_cursor_example.html")]
	#endif
	public class WorldSpaceCursorExample : MonoBehaviour
	{
		...
		private void SetPosition (Vector3 targetPosition)
		{
			...
			float distanceFromCamera = (targetPosition - Camera.main.transform.position).magnitude;
			distanceFromCamera = Mathf.Clamp (distanceFromCamera, minDistance, maxDistance);
			
			transform.position = Camera.main.transform.position + (ray.direction.normalized * distanceFromCamera);
		}
